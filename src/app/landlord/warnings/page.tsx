    ...
    <div>
      ...
      <svg width="260" height="120" viewBox="0 0 260 120">
        ...
        {[100, 75, 50, 25, 0].map((v, i) => (
          <g key={i}>
            <text x="0" y={i * 22 + 4} style={{ fontSize: '9px', fill: '#94a3b8' }}>{v}%</text>
            <line x1="26" y1={i * 22} x2="252" y2={i * 22} stroke="#f1f5f9" strokeWidth="1" />
          </g>
        ))}
        {/* Area fill */}
        ...
        {/* Dots */}
        {points.map((p, i) => (
          <circle key={i} cx={p.x + 26} cy={p.y} r="3.5" fill="#f97316" stroke="#ffffff" strokeWidth="1.5" />
        ))}
        {/* X axis labels */}
        {labels.map((l, i) => (
          <text key={i} x={i * 40 + 26} y="115" textAnchor="middle" style={{ fontSize: '9px', fill: '#94a3b8' }}>{l}</text>
        ))}
      </svg>
    </div>
  );
...
export default function LandlordWarningsPage() {
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '0' }}>
      ...
      {/* Alert Banner */}
      <div style={{ backgroundColor: '#fffbeb', border: '1px solid #fde68a', borderRadius: '10px', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', textAlign: 'left' }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#fef3c7', color: '#f59e0b', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', flexShrink: 0 }}>
            <LuTriangleAlert />
          </div>
          <div>
            <div style={{ fontSize: '0.875rem', fontWeight: 800, color: '#92400e' }}>Significant Change Detected</div>
            <div style={{ fontSize: '0.8rem', color: '#a16207', marginTop: '2px' }}>1 tenant showing a sharp decline in engagement</div>
          </div>
        </div>
        <button
          style={{ padding: '8px 18px', borderRadius: '8px', border: '1.5px solid #f59e0b', backgroundColor: '#ffffff', color: '#92400e', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}
      ...
      <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '24px 28px', marginBottom: '20px', boxShadow: 'var(--shadow-sm)', textAlign: 'left' }}>
        {/* Tenant Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '54px', height: '54px', borderRadius: '50%', backgroundColor: '#f1f5f9', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem' }}>
              👤
            </div>
            <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', backgroundColor: '#fff5f5', border: '1.5px solid #fca5a5', borderRadius: '8px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
          ...
          <div style={{ backgroundColor: '#f8fafc', borderRadius: '10px', padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                { icon: <LuClock />, signal: 'Response Timing', value: 'Slower', color: '#f59e0b', why: 'Slower responses May Indicate Lower Engagement.' },
                { icon: <LuRefreshCw />, signal: 'Follow -through', value: 'Partial', color: '#f59e0b', why: 'Incomplete follow-through can signal hesitance or uncertainty.' },
      ...
      {/* Insight Box */}
      <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '16px 20px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '14px', boxShadow: 'var(--shadow-sm)', textAlign: 'left' }}>
        <div style={{ width: '38px', height: '38px', borderRadius: '50%', backgroundColor: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', flexShrink: 0 }}>
          💡
        </div>
        <div>
      ...
      <div style={{ textAlign: 'left', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
          ...
          {/* Schedule Review */}
          <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '20px', boxShadow: 'var(--shadow-sm)' }}>
            <button
              style={{ width: '100%', padding: '12px 20px', borderRadius: '8px', border: 'none', backgroundColor: '#0a57e3', color: '#ffffff', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '16px' }}
          ...
          <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '20px', boxShadow: 'var(--shadow-sm)' }}>
            <button
              style={{ width: '100%', padding: '12px 20px', borderRadius: '8px', border: '1.5px solid #7c3aed', backgroundColor: '#ffffff', color: '#7c3aed', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '16px' }}
          ...
          {/* What Happens Next */}
          <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '20px', boxShadow: 'var(--shadow-sm)', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                  <LuCircleCheck style={{ color: '#10b981', fontSize: '0.95rem', flexShrink: 0 }} />
                  {item}
                </div>
              ))}
            </div>
            <button
              ...
              style={{ width: '100%', padding: '9px', borderRadius: '8px', border: '1.5px solid var(--border-color)', backgroundColor: '#ffffff', color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', ...(reviewed ? { backgroundColor: '#e6fbf3', borderColor: '#a7f3d0', color: '#10b981' } : {}) }}
            >
              <LuCircleCheck style={{ fontSize: '0.9rem', color: reviewed ? '#10b981' : 'var(--text-muted)' }} />
      ...
      {/* Footer Note */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', backgroundColor: '#f0f4ff', border: '1px solid #dbeafe', borderRadius: '8px' }}>
